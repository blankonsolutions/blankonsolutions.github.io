		...
		function renderNav() {
			return `
				<div class="nav">
					...
					<div class="nav-item ${state.page === 'vendors' ? 'active' : ''}" onclick="setPage('vendors')"><div>📝</div>심사</div>
					<div class="nav-item ${state.page === 'settings' ? 'active' : ''}" onclick="setPage('settings')"><div>⚙️</div>설정</div>
					<div class="nav-item ${state.page === 'logs' ? 'active' : ''}" onclick="setPage('logs')"><div>📜</div>로그</div>
				</div>
			`;
		}
						...
						${state.pendingVendors.map(v => `
							<div class="card">
								<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
									<div>
										<div style="font-weight:bold; font-size:17px;">${v.name}</div>
										<div style="font-size:12px; color:var(--text-sub);">${v.type} 사업자 • ${v.date} 신�				case 'vendor_detail':
					...
					return `
						<div class="padding">
							...
							<div style="display:grid; grid-template-columns: 1fr 1.5fr; gap:15px;">
								<button class="primaryBtn" style="background:#333;" onclick="setPage('reject_confirm', ${JSON.stringify(v)})">가입 반려</button>
			...
			switch (state.page) {
				...
				case 'reject_confirm':
					...
					`;; margin-bottom:10px;">반려자 확인</div>
								<input class="input" type="text" placeholder="성함 입력" value="${state.currentUser.name}" style="border:1px solid var(--border); padding:12px; border-radius:8px; background:var(--bg); color:#fff; width:100%; margin-bottom:10px;">
								<input class="input" type="text" placeholder="사원번호 입력" value="${state.currentUser.adminId}" style="border:1px solid var(--border); padding:12px; border-radius:8px; background:var(--bg); color:#fff; width:100%;">
							</div>
							...
							<button class="primaryBtn" style="margin-top:20px; background:var(--danger);" onclick="if(confirm('${rv.name} 업체를 정말로 반려하시겠습니까?')) { alert('반려 처리가 완료되었습니다.'); setPage('vendors'); }">네, 반려합니다</button>
							<button class="primaryBtn" style="margin-top:10px; background:transparent; color:var(--text-dark); border:1px solid var(--border);" onclick="setPage('vendor_detail', ${JSON.stringify(rv)})">취소</button>
						</div>
					`;
